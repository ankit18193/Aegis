# Aegis — Distributed AI Agent Execution Platform
...
1. **Google Antigravity is the Development Environment**:
   ...
   - Customizations and workflows are discovered from `.agents/` and `.antigravity/`.
2. **Zero Claude Code Dependency**:
   ...
   - Aegis has ZERO dependencies on Claude Code or Claude API keys.
3. **GStack is STRICTLY an Engineering Workflow Layer**:
   - GStack resides in `.agents/` and `.antigravity/skills/gstack/`.
   - It is **NOT** an Aegis application or runtime dependency.
   - Aegis production source code, Docker images, worker processes, API services, and infrastructure must **NEVER** import or depend on GStack.
4. **Engineering Rigor & Boundary Discipline**:
   - Aegis is a major distributed systems platform, not a simple CRUD application.
   - Before implementing any phase or major feature, always answer:
     1. **WHAT** are we building?
     2. **WHY** are we building it?
     3. **WHAT** architectural boundary does it affect?
     ...
     8. **HOW** will it behave under failure?
     9. **HOW** will it affect future phases?
5. **Preserve the Aegis Phase Roadmap**:
   - The Aegis 22-phase implementation roadmap is the absolute source of truth.
   - GStack operates **on top of** the roadmap to ensure disciplined delivery.
   - For every phase: Understand → Review Requirements → Challenge Assumptions → Review Architecture (`/plan-eng-review`) → Implementation Plan → Incremental Execution → Code Review (`/review`) → QA (`/qa`) → Regression Verification → Production-Readiness Review.
...
- **Search Before Building**: Check what exists before writing new code:
  - _Layer 1 (Tried & True)_: Standard battle-tested patterns.
  - _Layer 2 (New & Popular)_: Modern ecosystem approaches (scrutinize carefully).
  - _Layer 3 (First Principles)_: Deep understanding of the specific problem at hand.
- **The Reuse Ladder**:
...
## Available GStack Workflows in Antigravity

Invoke workflows via slash command (e.g. `/plan-eng-review`) or natural prompts:
...
## Lifecycle Phase Runbooks

Detailed runbooks are located in [.agents/phases/](file:///.agents/phases/):
